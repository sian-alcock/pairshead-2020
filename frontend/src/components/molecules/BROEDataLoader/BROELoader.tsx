...
const importApis = {
  ...
  updateSettings: (id: number, data: FormData) => 
    axios.put(`/api/global-settings-list/${id}`, data),
  createSettings: (data: FormData) => 
    axios.post('/api/global-settings-list/', data),
};
      ...
      try {
        ...
        if (clubsResponse.status === 'rejected' || eventsResponse.status === 'rejected') {
          ...
          if (clubsResponse.status === 'rejected') {
            errorMsg = clubsResponse.reason instanceof Error 
              ? clubsResponse.reason.message 
              : 'Failed to import clubs';
          } else if (eventsResponse.status === 'rejected') {
            errorMsg = eventsResponse.reason instanceof Error 
              ? eventsResponse.reason.message 
