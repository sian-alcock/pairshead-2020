...
interface CSVDataLoaderProps {
  ...
  // Injected by FeedbackModal when autoClose is enabled
  onAutoCloseTrigger?: () => void;
  isModalAutoClosing?: boolean;
}
...
const CSVDataLoader: React.FC<CSVDataLoaderProps> = ({
  ...
  // Injected by FeedbackModal
  onAutoCloseTrigger,
  isModalAutoClosing = false
}) => {
  ...
  const uploadMutation = useMutation<UploadResponse, UploadError, UploadVariables>({
    ...
    onSuccess: (data) => {
      ...
      // Trigger auto-close if modal supports it
      if (onAutoCloseTrigger) {
        onAutoCloseTrigger();
      }
    },
    onError: (error) => {
  ...
  return (
    <section className="csv-data-loader">
      {title && (
      ...
      <div className="csv-data-loader__upload-area">
        <CSVDragDrop
          ...
          disabled={uploadMutation.isPending || isModalAutoClosing}
      ...
      <div className="csv-data-loader__status">
        ...
        {uploadMutation.isError && (
          <div className="csv-data-loader__error">
            ...
            <button onClick={handleClearError} className="csv-data-loader__retry-button">
              Clear Error
            </button>
          </div>
        )}
