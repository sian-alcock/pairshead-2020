...
export default function CrewTimeCompareTable({
  ...
  const calculateRawTime = (crew: CrewProps, race: RaceProps): number | null => {
    const startTime = crew.times.find((time) => time.tap === "Start" && time.race?.id === race.id);
    const finishTime = crew.times.find((time) => time.tap === "Finish" && time.race?.id === race.id);
  ...
  const columns = useMemo<ColumnDef<CrewProps, any>[]>(() => {
    const baseColumns = [
      columnHelper.accessor("id", {
        header: "Id",
        cell: (info) => <Link to={`/crews/${info.getValue()}/edit`}>{info.getValue()}</Link>,
        enableSorting: true,
    ...
    races.forEach((race, idx) => {
      ...
      raceColumns.push(
        columnHelper.accessor(
          (row) => {
            const time = row.times.find((time) => time.tap === "Start" && time.race?.race_id === race.race_id);
      ...
      raceColumns.push(
        columnHelper.accessor(
          (row) => {
            const time = row.times.find((time) => time.tap === "Finish" && time.race?.race_id === race.race_id);
            return time?.time_tap || null;
          },
          {
      ...
      raceColumns.push(
        columnHelper.accessor((row) => calculateRawTime(row, race), {
          ...
          columnHelper.accessor(
            ...
            },
            {
              ...
              cell: (info) => {
                ...
                return (
                  <span className={`crew-time-compare__tag crew-time-compare__tag--${colorClass}`}>{display}</span>
                );
              },
  ...
  return (
    <>
      ...
      <div className="crew-time-compare__control-wrapper">
        <div className="">
          <div className="">
            <SearchInput value={globalFilter} onChange={setGlobalFilter} placeholder="Search all columns..." />
          </div>
        </div>
        <div className="">
          <div className="">
            <TextButton
              ...
              label={updateOverridesMutation.isPending ? "Saving..." : `Save changes (${pendingOverrides.size} crews)`}
            />
          </div>
        </div>
      </div>
        ...
        <div className="level-left">
          <div className="level-item">
            <p className="has-text-grey">
              ...
              {globalFilter && <span> (filtered from {crews.length} total)</span>}
            </p>
          </div>
        </div>
      </div>

      <TablePagination table={table} className="mt-4" showRowInfo={true} showPageSizeSelector={true} />
    </>
  );
}
