...
export default function CrewsTable({ onDataChanged }: CrewsTableProps) {
  ...
  const columns = useMemo<ColumnDef<CrewProps, any>[]>(() => [
    columnHelper.accessor("id", {
      header: "ID",
      cell: (info) => (
        <Link to={`/generate-results/crew-management-dashboard/${info.getValue()}/edit`} className="crews-table__cell crews-table__cell--id">
    ...
    columnHelper.accessor((row) => row.competitor_names || row.name, {
      ...
      header: "Name",
      cell: (info) => (
        <span className="crews-table__cell crews-table__cell--name">
          {info.getValue()}
        </span>
      ),
      enableSorting: true,
    ...
    columnHelper.accessor("bib_number", {
      header: "Bib",
      cell: (info) => (
        <span className="crews-table__cell crews-table__cell--bib">
          {info.getValue() || '--'}
        </span>
      ),
      enableSorting: true,
    ...
    columnHelper.accessor("event_band", {
      header: "Event",
      cell: (info) => (
        <span className="crews-table__cell crews-table__cell--event">
          {info.getValue() || '--'}
        </span>
      ),
      enableSorting: true,
    ...
    columnHelper.accessor((row) => row.club?.name || '--', {
      ...
      header: "Club",
      cell: (info) => (
        <span className="crews-table__cell crews-table__cell--club">
          {info.getValue()}
        </span>
      ),
      enableSorting: true,
    ...
    columnHelper.accessor("status", {
      header: "Status",
      cell: (info) => (
        <span className={`crews-table__cell crews-table__cell--status crews-table__cell--status-${info.getValue()?.toLowerCase().replace(' ', '-')}`}>
          {info.getValue() || '--'}
        </span>
      ),
      enableSorting: true,
    ...
    columnHelper.accessor("start_sequence", {
      header: "Start Seq",
      cell: (info) => (
        <span className="crews-table__cell crews-table__cell--sequence">
          {info.getValue() || '--'}
        </span>
      ),
      enableSorting: true,
    ...
    columnHelper.accessor("overall_rank", {
      header: "Overall Rank",
      cell: (info) => (
        <span className="crews-table__cell crews-table__cell--rank">
          {info.getValue() || '--'}
        </span>
      ),
      enableSorting: true,
    ...
    columnHelper.accessor("composite_code", {
      ...
      cell: (info) => (
        <span className="crews-table__cell crews-table__cell--code">
          {info.getValue() || '--'}
        </span>
      ),
      enableSorting: true,
    ...
    columnHelper.accessor("time_only", {
      header: "Time Only",
      cell: (info) => (
        <span className={`crews-table__cell crews-table__cell--boolean ${info.getValue() ? 'crews-table__cell--boolean-true' : 'crews-table__cell--boolean-false'}`}>
  ...
  return (
    <div className="crews-table">
      ...
      <div className={`crews-table__column-checkboxes ${showColumnToggles ? 'crews-table__column-checkboxes--expanded' : 'crews-table__column-checkboxes--collapsed'}`}>
