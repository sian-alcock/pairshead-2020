...
import { MarshallingDivision } from "../../../types/components.types";
import { useMarshallingDivisions } from "../../../hooks/useMarshallingDivisions";
...
const MarshallingDivisionsTable: React.FC = () => {
  ...
  // Fetch data
  const { data: fetchedData, isLoading, error } = useMarshallingDivisions();
  ...
  return (
    <div className="marshalling-table">
      <div className="marshalling-table__header">
        ...
        <p className="marshalling-table__description">
          Bottom ranges are calculated to be sequential. Each division starts where the previous one ends + 1.
        </p>
      ...
      {data.length === 0 && (
        <div className="marshalling-table__empty">No divisions found. Click "Add division" to create one.</div>
      )}
    </div>
  );
};
