...
export default function TimingOffsetManagerDetail() {
  ...
  const fetchData = async () => {
    try {
      ...
      const referenceRace = raceResponseData?.find(
        (race: { is_timing_reference: boolean }) => race.is_timing_reference
      );
      if (referenceRace) {
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    try {
      ...
      const data = {
        ...
        reference_race: referenceRaceId,
        target_race: targetRaceId
      };
        ...
        <div className="timing-offset-manager__container">
          {!isLoading && (
            <form onSubmit={handleSubmit} className="timing-offset-manager__form">
              ...
              {/* Offset Direction */}
              <div className="timing-offset-manager__field">
                <label className="">Offset Direction</label>
                <div className="flex gap-4">
                  <label className="flex items-center">
                    <input type="radio" checked={!isNegative} onChange={() => setIsNegative(false)} className="mr-2" />
                    <span className="text-sm">Target is ahead (+)</span>
                  </label>
                  <label className="flex items-center">
                    <input type="radio" checked={isNegative} onChange={() => setIsNegative(true)} className="mr-2" />
              ...
              <div className="">
                ...
                {message && (
                  <div
                    className={`timing-offset-manager__message ${
                      messageType === "success"
                        ? "timing-offset-manager__message--success"
                        : "timing-offset-manager__error"
                    }`}
                  >
              ...
              <div className="">
                <p>
                  <strong>Tip:</strong> If the target race's clock shows a time that's 2.5 seconds behind the reference
                  race, select "Target is behind (-)" and enter 0:02.50
                </p>
              </div>
            </form>
          )}
