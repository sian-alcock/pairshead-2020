...
export default function NumberLocationEdit() {
  ...
  const { data, loading } = useFetchData(`/api/number-locations/${routeParams.id}`);
  ...
  const handleDelete = () => {
    axios
      .delete(`/api/number-locations/${routeParams.id}`)
      .then(() => history.push("/set-number-locations"))
  ...
  const handleSubmit = (e: React.SyntheticEvent) => {
    ...
    axios
      .put(`/api/number-locations/${routeParams.id}`, formData)
      .then(() => history.push("/set-number-locations"))
          ...
          {!loading && (
            <form onSubmit={handleSubmit} className="has-text-centered">
              <div className="field">
                ...
                <input className="input" name="club" id="club" defaultValue={formData.club} onChange={handleChange} />
              </div>

              <div className="field">
                <label className="label" htmlFor="numberLocation">
                  Number location
                </label>
                <input
                  className="input"
